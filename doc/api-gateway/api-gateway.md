...
| CORS | **two policies** (per-request in `main.ts`): the public Delivery API (`/v1/projects/*`) **reflects any origin with credentials off** (Bearer-key reads only — Contentful CDA model, customer apps fetch browser-side); everything else is an exact-origin allowlist `CORS_ORIGINS` with credentials (dev `localhost:3000,3001`; prod wriven.tech + www + admin.wriven.tech + www.admin.wriven.tech). No-Origin requests (curl, server-to-server) pass untouched |
