...
## Content model
- **content_types** — user-defined schema: `fields` is a `FieldDef[]` JSONB (8 types incl. `reference` + `media`, with `unique`/`multiple` toggles). Soft-deleted; projects start **empty** (no seeded types — the former starter `Post` seeding was removed).
...
- **content_revisions** — one per write; restore records a new revision. Pruned to the plan's `revisionsPerEntry` cap (specs/15).

Also owned by `core_svc` (see their own diagrams): `usage_buckets` (below), `ai_generations` + `ai_profiles` ([10-ai-generation-flow](./10-ai-generation-flow.md)), and `support_tickets` (+ messages/attachments — [support-ticket/](../support-ticket/)).

## Entry lifecycle
`draft → published → archived` (+ soft delete). Each transition writes a revision. **Publish** is the side-effect hub: CDN tag-purge + webhook fan-out (`entry.published`/`unpublished`/`deleted` — `entry.published` refires on **every save of an already-published entry**, not just the first transition). Only `published` entries are visible to the Delivery API.

## Delivery API (public reads)
- Auth = **API key** (`wrk_live_` / `wrk_preview_` / `wrk_admin_`), not a user JWT. `ApiKeyGuard` resolves project scope; key scope drives `preview`→drafts.
...
## Media (R2, keys-only)
Presign → direct PUT to R2 → persist the object **key** (never a URL) in `media_assets` → delivery reconstructs the public URL at read. Per-workspace quota is **plan-driven** (`storageMb` via entitlements, resolved at presign) + fixed per-file caps enforced at presign.
