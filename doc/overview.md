...
| Core Service | NestJS 11 (TCP microservice) | CMS: content types, entries, media, **AI generation (in-process)** |
| AI Service | FastAPI (Python) | Deferred skeleton; extraction target for AI gen (HTTP when split out) |
...
│   └── ai-service/      # FastAPI (deferred skeleton — AI gen lives in core for now)
├── libs/shared/
