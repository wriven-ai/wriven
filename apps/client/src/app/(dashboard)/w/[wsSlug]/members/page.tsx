...
import { Clock, Mail, RefreshCw, Send, Shield, Trash2, Users, X } from 'lucide-react';
...
import { WorkspacePermissionsTable } from '@/components/auth/permissions-matrix-table';
import type {
...
import { WorkspaceMembersSkeleton, MemberRows } from '@/components/skeleton/members-skeleton';
...
function MembersInner() {
  ...
  return (
    <div className="space-y-6 text-left" id="members-workspace">
      {/* Page Header */}
      <div className="border-b border-brand-border pb-5">
        <h1 className="font-display font-medium text-2xl sm:text-3xl text-text-primary tracking-tight">
          Workspace <span className="font-normal italic text-brand-secondary">Members</span>
        </h1>
        <p className="text-sm sm:text-base font-mono text-text-muted mt-1.5 leading-relaxed">
          {`// Manage who can access ${currentWorkspace?.name ?? 'this workspace'} and review role permissions`}
        </p>
      </div>
      ...
      <Tabs defaultValue="members" className="w-full space-y-6">
        <TabsList className="bg-brand-surface-soft/60 border border-brand-border p-1 rounded-xl h-auto inline-flex max-w-full overflow-x-auto">
          ...
          >
            <Users className="w-4 h-4 mr-1.5 text-brand-secondary" />
            Members & Invites
          </TabsTrigger>
          <TabsTrigger
            value="permissions"
            className="px-4 py-2 rounded-lg text-xs font-mono font-bold tracking-wider uppercase data-active:bg-brand-surface data-active:text-text-primary data-active:shadow-xs transition-all"
          >
            <Shield className="w-4 h-4 mr-1.5 text-brand-accent" />
                        ...
                        <div key={member.id} className="flex items-center justify-between gap-3 p-3 border border-brand-border bg-brand-surface-soft/40 rounded-xl">
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                {isSelf && (
                                  <span className="text-xs font-bold px-1.5 py-0.5 rounded uppercase bg-brand-surface text-text-muted border border-brand-border">You</span>
                                )}
                              </div>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            {editable ? (
                              ...
                              <span className={`px-2 py-0.5 rounded text-xs font-mono font-semibold uppercase border ${ROLE_BADGE[member.role] ?? ROLE_BADGE.member}`}>
                                {member.role}
                              </span>
                            )}
                            {editable && (
