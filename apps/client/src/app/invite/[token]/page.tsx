    ...
    <div className="flex min-h-screen items-center justify-center bg-brand-bg px-4">
      <div className="w-full max-w-md rounded-xl border border-brand-border bg-brand-surface p-6 shadow-xs sm:p-8">
        {isLoading || status === 'loading' ? (
          <div className="flex items-center justify-center gap-2 py-10 font-mono text-xs text-text-muted">
      ...
      {emailMismatch ? (
        ...
        <div className="space-y-2">
          <Link
            ...
            className="inline-flex w-full items-center justify-center gap-1.5 rounded-lg bg-brand-accent px-4 py-2.5 font-mono text-xs font-bold text-white transition-all hover:bg-brand-accent-hover"
          >
            Create your account
          </Link>
        ...
        <button
          onClick={onAccept}
          disabled={accepting}
          className="inline-flex w-full items-center justify-center gap-1.5 rounded-lg bg-brand-accent px-4 py-2.5 font-mono text-xs font-bold text-white transition-all hover:bg-brand-accent-hover disabled:opacity-60"
        >
          {accepting ? (
...
function Dead({
  ...
  return (
    <div className={inline ? 'space-y-2' : 'space-y-3 text-center'}>
      ...
      <p className="font-mono text-[10px] leading-relaxed text-text-muted">{body}</p>
    </div>
  );
}
