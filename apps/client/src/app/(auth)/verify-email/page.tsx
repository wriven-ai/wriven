  ...
  if (state === 'no-token') {
    return (
      <div className="text-center space-y-3">
        ...
        <Link href="/login" className="inline-block text-xs text-brand-accent font-semibold hover:underline">
          Go to sign in
        </Link>
      </div>
    );
  ...
  if (state === 'verifying') {
    return (
      <div className="flex flex-col items-center gap-3 py-4 text-text-muted">
        ...
        <p className="text-xs font-mono">Verifying your email…</p>
      </div>
    );
  }
...
export default function VerifyEmailPage() {
  return (
    <Shell>
      <Suspense
        fallback={
          <p className="text-center text-xs font-mono text-text-muted">Loading…</p>
        }
      >
        <VerifyInner />
      </Suspense>
