...
/**
 * Interactive pricing surface: billing-cycle toggle, plan cards, comparison
 * matrix, FAQ. `initialPlans` arrives from the server-rendered fetch
 * (`lib/server/plans`) so the first paint shows real plans with no skeleton;
 * when it is null (gateway unreachable at render time) the client query
 * takes over with the loading/error states.
 */
export function PricingClient({ initialPlans }: { initialPlans: PlanView[] | null }) {
...
// ── Page heading + billing-cycle toggle ─────────────────────────────────────
  ...
  return (
    ...
    >
      ...
      <p className="text-text-secondary text-sm leading-relaxed font-light">
        Start weaving content completely free, no credit card required. Lock in
        annual billing for a stable 10% savings.
      </p>
