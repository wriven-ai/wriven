...
export class AdminProjectsController {
  ...
  list(@Query() query: AdminProjectsQueryDto) {
    return sendWithTimeout(this.auth, ADMIN_PATTERNS.PROJECTS_LIST, query);
  }

  @Get(':id')
  get(@Param('id') id: string) {
    return sendWithTimeout(this.auth, ADMIN_PATTERNS.PROJECTS_GET, { id });
  }

  /** Aggregated per-project usage (core-owned tables: content, media, keys, webhooks, AI). */
  @Get(':id/usage')
  usage(@Param('id') id: string) {
    return sendWithTimeout(this.core, ADMIN_PATTERNS.PROJECT_USAGE, { projectId: id });
  }
  ...
  remove(@Param('id') id: string) {
    return sendWithTimeout(this.auth, ADMIN_PATTERNS.PROJECTS_DELETE, { id });
  }
}
